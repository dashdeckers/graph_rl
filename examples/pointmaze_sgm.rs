...
fn main() -> Result<()> {
    ...
    //// Create the PointMazeEnv Environment for Training ////

    let mut pointmaze = *PointMazeEnv::new(PointMazeConfig::default())?;


    //// Create DDPG_SGM Algorithm (with a larger buffer) ////

    let mut ddpg_sgm_config = DDPG_SGM_Config::small();
    ddpg_sgm_config.ddpg.replay_buffer_capacity = 100_000;
    ddpg_sgm_config.ddpg.training_batch_size = 100;
    let mut ddpg_sgm = *DDPG_SGM::from_config(
        &device,
        &ddpg_sgm_config,
        pointmaze.observation_space().iter().product::<usize>(),
        pointmaze.action_space().iter().product::<usize>(),
    )?;
    ...
    if args.gui {
        ...
        SgmGUI::<DDPG_SGM<PointMazeEnv>, PointMazeEnv, _, _>::open(
            ParamEnv::AsEnvironment(pointmaze),
        ...
        run_experiment_off_policy::<DDPG_SGM<PointMazeEnv>, PointMazeEnv, _, _>(
            &args.name,
            100,
            ParamEnv::AsEnvironment(pointmaze),
            ParamAlg::AsAlgorithm(ddpg_sgm),
